...
public class DriverSignIn extends AppCompatActivity {
                            ...
                            public void onComplete(Task<AuthResult> task) {
                                if (!task.isSuccessful()) {
                                    try {
                                        ...
                                        Toast.makeText(DriverSignIn.this, "Неверный email", Toast.LENGTH_SHORT).show();
                                    }
                                    catch (FirebaseAuthInvalidCredentialsException e) {
                                        Toast.makeText(DriverSignIn.this, "Неверный пароль", Toast.LENGTH_SHORT).show();
                                    }
                                    catch (Exception e) {
                                        Toast.makeText(DriverSignIn.this, "Ошибка входа", Toast.LENGTH_SHORT).show();
                                        Log.d("vhod", "" + e.getMessage());
                                    }
                                }
                                else {
                                    //Вход удачно
                                    Toast.makeText(DriverSignIn.this, "Успешный вход", Toast.LENGTH_SHORT).show();
                            ...
                            public void onComplete(Task<AuthResult> task) {
                                if (!task.isSuccessful()) {
                                    try {
                                        ...
                                        Toast.makeText(DriverSignIn.this, "Данный email уже существует", Toast.LENGTH_SHORT).show();
                                    }
                                    catch (FirebaseAuthWeakPasswordException e) {
                                        Toast.makeText(DriverSignIn.this, "Пароль должен быть не менее 6 символов", Toast.LENGTH_SHORT).show();
                                    }
                                    catch (Exception e) {
                                        Toast.makeText(DriverSignIn.this, "Ошибка регистрации", Toast.LENGTH_SHORT).show();
                                        Log.d("reg", "" + e.getMessage());
                                    }
                                }
                                else {
                                    // Регистрация прошла успешна
                                    Toast.makeText(DriverSignIn.this, "Регистрация прошла успешна", Toast.LENGTH_SHORT).show();
                                    FirebaseUser user = auth.getCurrentUser();
                                    startActivity(new Intent(DriverSignIn.this, DriverMapsActivity.class));
                                }
                            }
    ...
    //Метод для перехода между Регистрация/Вход
    public void ChangeLogin(View view) {
        if (isLoginModeActive){
            isLoginModeActive = false;
            textViewTitle.setText("Регистрация водителя");
            ...
            isLoginModeActive = true;
            textViewTitle.setText("Аудентификация водителя");
